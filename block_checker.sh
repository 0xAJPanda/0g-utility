...
CONFIG_RPC="https://evmrpc-testnet.0g.ai"
FALLBACK_RPC="https://evmrpc-testnet.0g.ai"
...
while true; do 
    ...
    NETWORK_RESPONSE=$(curl -s -m 5 -X POST "$ACTIVE_RPC" -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}')
    latestBlockHex=$(echo "$NETWORK_RESPONSE" | jq -r '.result' 2>/dev/null)

    if [[ "$NETWORK_RESPONSE" == *"rate limit"* || "$NETWORK_RESPONSE" == *"Too Many Requests"* || -z "$latestBlockHex" || "$latestBlockHex" == "null" ]]; then
        if [ "$RPC_SWAPPED" = false ]; then
            echo -e "\nSwitching to fallback RPC: \033[1;34m$FALLBACK_RPC\033[0m"
            ACTIVE_RPC="$FALLBACK_RPC"
            RPC_SWAPPED=true
            continue
        fi
    fi
